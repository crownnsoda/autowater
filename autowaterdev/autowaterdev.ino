...
int waterLevel = 1; // hard set to 1 until water level sensor connected
//int waterlevel();//comment out above line and uncomment this line when water sensor is connected
...
void flowActivate()
{
	...
	else
	{
	    ...
	    return indicateBlue;
	    return indicateRed;
	    return indicatePurple;
	    return indicateGreen;
}
...
//waterLevel = analogRead(waterSourcePin); //uncomment line when water level sensor attached

while (waterLevel > 0)
{
	delay(soilCheckInterval);//take a reading every x ms's
	
	readSoil();//runs readSoil function to get sensor values; returns valBlue, valRed, valPurple, valGreen
	
	flowActivate(); //runs fuction to activate pump and solenoids based on sensor values    
    ...
    Serial.print("valPurple ");
    Serial.println(valPurple);
